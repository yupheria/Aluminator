        ...
        <div id="page-wrapper" >
            <div id="page-inner">
			  ...
			  foreach($user_profile as $up){
				  echo 'Name:- '. $up->firstname.'&nbsp;'.$up->lastname.'<br/>';
				 echo 'Gender:- '.$up->gender.'<br/>';
				 echo 'Email:- '.$up->email.'<br/>';
				 echo 'About:- '. $up->about.'<br/> <br/>';
				 
				 //checking the status of freind request for sender
				  	$this->load->model('user_model');
					$id = $up->id;
				   $Freindcheck = $this->user_model->check_friend($id);
				   $Freindcheckotherside = $this->user_model->check_friend_otherside($id);
				   if($Freindcheck){
					   
					   foreach($Freindcheck as $f_r){
						     echo $f_r->status;
					   }
				   }
				   
				   //checking the status of freind request for reciver
				   elseif($Freindcheckotherside){
					     foreach($Freindcheckotherside as $f_r_os){
						     echo $f_r_os->status;
						 }
				   }
				   else{
				  echo anchor('users/add_friend/'.$up->id.'/'.$up->firstname. $up->lastname.'','Add Friend',array('class'=>'btn btn-primary'));
				   }
				   
				   
				 echo '</div>';
				 echo '<div class="col-lg-6  image"><img class=" img-responsive" src="'.base_url().'/webroot/images/user_images/'.$up->image.'">';
